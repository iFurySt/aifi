#!/usr/bin/env python3
"""Collect YouTube transcripts through InsightsTube's online endpoint."""
...
PROVIDER_NAME = "InsightsTube"
PROVIDER_SLUG = "insightstube"
PROVIDER_ENDPOINT = "https://insightstube.com/api/transcript"
...
def provider_url(video_url: str) -> str:
    return f"{PROVIDER_ENDPOINT}?{urllib.parse.urlencode({'url': video_url})}"
...
def fetch_transcript(video: dict[str, Any], timeout: int) -> tuple[int | None, dict[str, Any] | None, str | None]:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "referer": "https://insightstube.com/",
...
def write_transcript(args: argparse.Namespace, video: dict[str, Any], provider_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "provider": {
            "name": PROVIDER_NAME,
            "endpoint": provider_url(video["url"]),
        ...
        "transcript": {
            ...
            "segments": segments,
            "language": {
                "lang": provider_payload.get("lang"),
                "langName": provider_payload.get("langName"),
...
def collect(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for video in selected:
        ...
        row = {
            ...
            "url": video["url"],
            "provider": PROVIDER_NAME,
            "provider_url": provider_url(video["url"]),
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(segments, list):
                row["segments_returned"] = len(segments)
                row["characters_returned"] = sum(len(str(segment.get("text") or "")) for segment in segments if isinstance(segment, dict))
            row["video_title_returned"] = payload.get("title")
            row["language_returned"] = payload.get("langName") or payload.get("lang")
        try:
            if status_code == 200 and isinstance(payload, dict) and isinstance(payload.get("segments"), list):
...
def main() -> None:
    args = parse_args()
    rows = collect(args)
    manifest_path = args.target_dir / "evidence" / "media" / "provider-probes" / "insightstube-manifest.json"
