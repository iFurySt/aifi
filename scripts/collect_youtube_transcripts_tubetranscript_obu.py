#!/usr/bin/env python3
"""Collect YouTube transcripts from TubeTranscript browser pages."""
...
PROVIDER_NAME = "TubeTranscript"
PROVIDER_SLUG = "tubetranscript"
PROVIDER_BASE_URL = "https://tubetranscript.com/en"
...
def provider_url(video_id: str) -> str:
    return f"{PROVIDER_BASE_URL}/watch?v={video_id}"
...
JSON.stringify({
  ...
  transcriptText: (document.querySelector("#main-transcript-content")?.innerText || "").trim(),
  segments: [...document.querySelectorAll(".transcript-group-box")]
    .map((node, index) => ({
      index,
      time: (node.querySelector(".transcript-time")?.innerText || "").trim(),
      text: (node.querySelector(".transcript-text")?.innerText || "").trim()
    }))
...
def setup_obu(args: argparse.Namespace) -> None:
    ...
    run([*obu_base(args), "name-session", "--name", "Terry TubeTranscript Provider - OBU"], timeout=30)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--obu-session-id", default="terry-tubetranscript-provider")
...
def main() -> None:
    args = parse_args()
    rows = collect(args)
    manifest_path = args.target_dir / "evidence" / "media" / "provider-probes" / "tubetranscript-manifest.json"
