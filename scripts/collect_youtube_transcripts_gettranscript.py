#!/usr/bin/env python3
"""Collect YouTube transcripts through GetTranscript's online endpoint."""
...
PROVIDER_NAME = "GetTranscript"
PROVIDER_SLUG = "gettranscript"
PROVIDER_ENDPOINT = "https://gettranscript.app/api/video/transcript"
...
def fetch_transcript(video: dict[str, Any], timeout: int) -> tuple[int | None, dict[str, Any] | None, str | None]:
    query = urllib.parse.urlencode({"videoId": video["id"], "format": "json"})
...
def normalize_segments(provider_payload: dict[str, Any]) -> list[dict[str, Any]]:
    segments = provider_payload.get("transcript")
...
def collect(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for video in selected:
        ...
        if isinstance(payload, dict):
            row["provider_message"] = payload.get("detail") or payload.get("error") or payload.get("message")
            transcript = payload.get("transcript")
            if isinstance(transcript, list):
                row["segments_returned"] = len(transcript)
                row["characters_returned"] = sum(len(str(segment.get("text") or "")) for segment in transcript if isinstance(segment, dict))
        try:
...
def main() -> None:
    args = parse_args()
    rows = collect(args)
    manifest_path = args.target_dir / "evidence" / "media" / "provider-probes" / "gettranscript-manifest.json"
