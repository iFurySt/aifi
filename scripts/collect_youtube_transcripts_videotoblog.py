#!/usr/bin/env python3
"""Collect YouTube transcripts through VideoToBlog's no-login transcript endpoint."""
...
PROVIDER_NAME = "VideoToBlog"
PROVIDER_SLUG = "videotoblog"
PROVIDER_PAGE = "https://videotoblog.ai/tools/youtube-to-transcript"
PROVIDER_ENDPOINT = "https://videotoblog.ai/api/getYtTranscript"
...
def post_transcript(video_url: str, lang: str, timeout: int) -> tuple[int | None, dict[str, Any] | None, dict[str, str], str | None]:
    body = json.dumps({"videoUrl": video_url, "lang": lang, "trackCtaClick": False}).encode("utf-8")
...
def write_transcript(args: argparse.Namespace, video: dict[str, Any], payload: dict[str, Any], headers: dict[str, str]) -> dict[str, Any]:
    ...
    out = {
        ...
        "provider": {
            ...
            "extraction": "Direct POST endpoint from VideoToBlog web tool",
            "selected_language": payload.get("selectedLanguage"),
            "content_type": headers.get("Content-Type") or headers.get("content-type"),
        },
        "video_index": video.get("index"),
        "video": video,
        "provider_video_info": payload.get("videoInfo"),
        "transcript": {
            "videoId": video["id"],
            "title": video["title"],
            "text": text,
...
def collect(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for video in select_videos(args):
        ...
        if payload:
            ...
            row["unavailable_reason"] = payload.get("unavailableReason")
            row["segments_returned"] = len(payload.get("transcript") or [])
        try:
...
def main() -> None:
    args = parse_args()
    rows = collect(args)
    manifest_path = args.target_dir / "evidence" / "media" / "provider-probes" / "videotoblog-manifest.json"
